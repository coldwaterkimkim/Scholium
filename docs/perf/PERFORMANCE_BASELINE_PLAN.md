...
- `semantic_guide_ready`: compact parser digest 기반 DocumentGuide와 모든 rendered page의 PageGuide가 준비됐다.
- `viewer_ready`: render + parser map + full required Semantic Guide + compatibility summary가 준비됐다. worklist/processing에서 viewer 진입 기준이다.
...
`GET /api/documents/{document_id}/processing`에서는 기존 호환용 `ready_for_viewer`와 함께 `parser_map_ready_pages`, `semantic_guide_ready`, `viewer_ready`, `page_context_ready_pages`, `document_context_ready`, `page_guide_count`, `semantic_guide_stage`, chunk counters를 본다.
