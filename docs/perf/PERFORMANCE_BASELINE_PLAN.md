...
- `semantic_guide_ready`: compact parser digest 기반 DocumentGuide와 모든 rendered page의 required Page Guide/Wrap-up이 준비됐다.
- `viewer_ready`: render + parser map + required Semantic Guide(Page Guide/Wrap-up 포함) + compatibility summary가 준비됐다. worklist/processing에서 viewer 진입 기준이다.
...
`GET /api/documents/{document_id}/processing`에서는 기존 호환용 `ready_for_viewer`와 함께 `parser_map_ready_pages`, `semantic_guide_ready`, `viewer_ready`, `page_context_ready_pages`, `document_context_ready`, `page_guide_count`, `semantic_guide_stage`, chunk counters를 본다. `page_guide_count`는 top-edge Page Guide와 bottom-edge Wrap-up이 함께 있는 required page record 수로 해석한다.
...
- `codex_cli_semantic_guide_call_count` for legacy single-call only
- `page_element_count`
- `page_guide_count`
