...
-> backend resolves exact SelectionTarget from the selected bbox
...
- Scholium resolves the exact selected target first, then explains that selected region on demand using page/document context that was prepared earlier.
...
Selected Explanation panel sections are:
...
`key_concept_detail` is intentionally not a section. If the selected target is a key concept, the concept definition belongs in `What this is`. `Common Confusion` means a useful distinction inside this learning material, not generic dictionary ambiguity.
...
- `selection_target`, the exact resolved target of the user's selected bbox
- selected bbox and page identity
- matched page elements as surrounding context or near-exact target support
- nearby text blocks as context
- page role and page summary as macro page context
- compact Page Guide/Wrap-up subset when available as macro orientation/review context
- brief document context when available as macro document context
- Semantic Guide DocumentGuide brief when available
- related page candidates
- source candidates
- a context hash for caching and reproducibility

Information hierarchy for Selection Explanation is strict: `selection_target` is primary, `selected_bbox` is the user's actual area, page elements/text blocks are context, and PageGuide/Wrap-up/DocumentGuide are macro context. Page elements must not override exact selected text unless the bbox is a near-exact element match.
...
| `SelectionTarget` | Deterministic target resolved from the selected bbox. It may be exact text, a page element, multiple elements, visual crop, mixed, or unknown. |
