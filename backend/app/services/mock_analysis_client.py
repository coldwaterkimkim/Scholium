...
class MockAnalysisClient:
    ...
    def run_pass1(
        ...
        return self._wrap(
            "pass1",
            {
                ...
                "wrap_up": self._wrap_up(response_language="ko"),
    ...
    def run_page_guide_chunk(
        ...
        return self._wrap(
            "page_guide_chunk",
            {
                ...
                "page_guides": [
                    self._semantic_page_guide(
                        document_id=document_id,
                        page_number=page_number,
                        response_language=response_language,
                        page_role=(
                            "Introduces the page's role in the mock learning sequence."
                            if response_language == "en"
                            else "Mock 학습 흐름에서 이 페이지의 위치를 잡아준다."
                        ),
                        one_line_thesis=(
    ...
    def _wrap_up(self, *, response_language: str) -> dict[str, Any]:
        if response_language == "en":
            return {
                "logic_flow": [
                    "Locate the page in the document flow.",
                    "Read the visible PDF content directly.",
                    "Use selection explanation only for confusing regions.",
                ],
                "study_focus": "Review how the page role and the selected-region workflow fit together.",
                "must_remember": [
                    "The proactive guide is only orientation and wrap-up.",
                    "Detailed explanation stays on demand.",
                ],
        ...
        return {
            "logic_flow": [
                "페이지가 문서 흐름에서 맡는 역할을 확인한다.",
                "PDF 본문을 직접 읽는다.",
                "막히는 영역만 선택 설명으로 확인한다.",
            ],
            "study_focus": "페이지 역할과 선택 설명 흐름이 어떻게 나뉘는지 확인한다.",
            "must_remember": [
                "proactive guide는 방향 잡기와 마무리만 담당한다.",
                "세부 설명은 사용자가 선택한 영역에서만 만든다.",
            ],
            "next_slide_connection": None,
        }

    def _semantic_page_guide(
        self,
        *,
        document_id: str,
        page_number: int,
        ...
        return {
            ...
            "wrap_up": self._wrap_up(response_language=response_language),
        }
